...
Отдельно проверил жалобу «и на `8081` тоже не работает» — пересобрал `web`+`nginx` с нуля и прогнал реальный браузерный сценарий (Playwright, настоящий Chrome) через `localhost:8081`: регистрация → логин → `/me` → `/chats` — всё `200`/`201`, редирект на экран чатов. Проблема не воспроизвелась — судя по всему, тестировали на ещё не пересобранном образе `nginx` (до фикса порядка `set`/`rewrite`) или с закэшированным состоянием. После пересборки `8081` рабочий.
