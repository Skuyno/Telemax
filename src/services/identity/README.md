...
- Refresh-токен не ротируется. `token_version` проверяется при рефреше и увеличивается в `POST /me/password` — смена пароля отзывает все ранее выданные refresh-токены (текущий access остаётся рабочим до истечения своих 15 минут).
